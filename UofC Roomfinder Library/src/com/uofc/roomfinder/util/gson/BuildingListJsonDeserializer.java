	...
	public List<Building> deserialize(JsonElement json, Type arg1, JsonDeserializationContext arg2) throws JsonParseException {
		...
		for (int i = 0; i < featureArray.size(); i++) {
			...
			for (int j = 0; j < geometryElements.size(); j++) {
				lat += geometryElements.get(j).getAsJsonArray().get(0).getAsDouble();
				lng += geometryElements.get(j).getAsJsonArray().get(1).getAsDouble();
			}
			lat = lat / geometryElements.size();
			lng = lng / geometryElements.size();
